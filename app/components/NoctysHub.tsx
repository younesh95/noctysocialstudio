...
import { EditorView, GeneratorView, TemplatesView } from "./StudioViews";
import { CalendarView, CreationsView } from "./SpaceViews";
import { NETWORKS, STATUS_LABELS, VIEW_LABELS } from "../lib/noctys";
import type { AppView, Creation, Task, TaskDraft, WorkStatus, WorkspaceData } from "../lib/types";

export function NoctysHub() {
  const [active, setActive] = useState<AppView>("templates");
  const [data, setData] = useState<WorkspaceData>({ tasks: [], creations: [] });
  ...
  const navigate = (view: AppView) => { setActive(view); setMobileNav(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const view = () => {
    ...
    switch (active) {
      case "templates": return <TemplatesView onCreated={onCreated} notify={notify}/>;
      ...
      case "creations": return <CreationsView creations={data.creations} onOpen={setOpenCreation}/>;
  ...
  return (
    <main className="app-shell">
      ...
      {openCreation&&<div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Détail de la création"><button className="modal-dismiss" onClick={()=>setOpenCreation(undefined)} aria-label="Fermer le détail"/><div className="creation-modal">
        ...
        <div className={`creation-detail-visual cover-${openCreation.template}`}><div className="cover-grid"/><img src="/noctys-logo.webp" alt="Logo NOCTYS"/><span>{NETWORKS[openCreation.network].short}</span></div>
        <div className="creation-detail-copy"><span className="eyebrow">{NETWORKS[openCreation.network].label} · {openCreation.kind}</span><h2>{openCreation.title}</h2><div className={`status-pill status-${openCreation.status}`}>{STATUS_LABELS[openCreation.status]}</div><p>{readableBody(openCreation.body)}</p><button className="primary" onClick={()=>{setOpenCreation(undefined);navigate(openCreation.kind==="texte"?"editor":"templates")}}>Modifier dans le Studio</button></div>
      </div></div>}
    </main>
  );
}
...
function readableBody(body: string) {
  try {
    const parsed = JSON.parse(body) as { title?: string; opponent?: string };
    return `${parsed.title ?? "Création NOCTYS"}${parsed.opponent ? ` · ${parsed.opponent}` : ""}`;
  } catch { return body; }
}
