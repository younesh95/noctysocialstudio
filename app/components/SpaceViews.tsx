        ...
        {filtered.map((creation) => (
          <button className="creation-card" key={creation.id} onClick={() => onOpen(creation)}>
            <div className={`creation-cover cover-${creation.template}`}>
              ...
              {creation.kind === "image" ? <img src="/noctys-logo.webp" alt=""/> : <FileText size={34}/>} 
              <strong>{creation.template.startsWith("ai-") ? "AI CONCEPT" : creation.template === "match-night" ? "MATCH NIGHT" : "NOCTYS COPY"}</strong>
            </div>
